================================================= HLD MINIMUM Query on Edges [single edge update(type replace value)] ===============================
=================================================================================================================================
...
int query_up(int l,int u) // l is ancestor of u
{
        ...
        while(nxt[u]!=nxt[l])
        {
                res = min(res , query(1,1,n, tin[nxt[u]] , tin[u]) );
                u = up[nxt[u]][0]; //next heavy chain e jump
        }

        res = min(res , query(1,1, n , tin[l]+1 , tin[u]) ); // l excluded , cause its not vertex rather it is edge sum query and each edge value are stored at down vertex of the edge [root theke takale down jedike]

        return res;
}

int minimum_query_hld(int u,int v)
{
        if(is_ancestor(u,v))
        {
                return query_up(u,v);
        ...
        return min(x,y) ; // l is already excluded
}
...
int32_t main()
{
        ...
        while(q--)
        {
                ...
                if(a==1)
                {
                        //update is replacing [u-v] edge value to x
                        int u,v,x;
                        cin>>u>>v>>x; // there must be an edges between u and v , else it will update all the edges in path from u to v which is not required [since update are basically adding ]

                        int val = minimum_query_hld(u,v); // there is only one edge between u,v and so there is only one value , so sum query and minimum query will give same ans

                        update_query_hld(u,v,-val); // clearing
                ...
                else
                {
                        int u, v; // [u!=v]
                        cin>>u>>v;
                        cout<<minimum_query_hld(u,v)<<endl;
                }
        }
